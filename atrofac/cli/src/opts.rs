...
pub struct Fan {
    ...
    pub gpu: String,
    /// The System fan curve. If not given, will set the fan speed to minimum. Must be a list of
    /// comma separated entries (8 entries). Each entry looks like this:
    /// <DEGREES>c:<PERCENTAGE>%. Example: 0c:0%,40c:0%,50c:0%,60c:0%,70c:31%,80c:49%,90c:56%,100c:56%.
    #[structopt(long = "system", default_value = "")]
    pub system: String,
}
