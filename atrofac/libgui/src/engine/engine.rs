...
impl Engine {
    ...
    pub fn apply(&mut self) -> Result<ApplyInfo, AfErr> {
        if let Some(active_plan) = self.active_plan() {
            ...
            if let (Some(cpu), Some(gpu), Some(system)) = (&active_plan.cpu_curve, &active_plan.gpu_curve, &active_plan.system_curve) {
                // plan and fan curve
                let cpu = convert_to_curve(FanCurveDevice::Cpu, cpu)?;
                let gpu = convert_to_curve(FanCurveDevice::Gpu, gpu)?;
                let system = convert_to_curve(FanCurveDevice::System, system)?;
                atk.set_power_plan(active_plan.plan.into())?;
                atk.set_fan_curve(&cpu)?;
                atk.set_fan_curve(&gpu)?;
                atk.set_fan_curve(&system)?;
                info!(
                    "Power plan updated with custom fan curve: {:?}; CPU {}; GPU {}; System {}.",
                    active_plan.plan,
                    cpu.to_string(),
                    gpu.to_string(),
                    system.to_string()
                );
            } else if let (Some(cpu), Some(gpu)) = (&active_plan.cpu_curve, &active_plan.gpu_curve) {
                // plan and fan curve (without system)
                let cpu = convert_to_curve(FanCurveDevice::Cpu, cpu)?;
                let gpu = convert_to_curve(FanCurveDevice::Gpu, gpu)?;
                atk.set_power_plan(active_plan.plan.into())?;
                atk.set_fan_curve(&cpu)?;
